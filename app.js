 ...
 * Automatización modular orientada a la consistencia multimodular y exportaciones ERP.
 */
...
// Configurar el worker global de PDF.js si la librería está disponible
...
function initApplication() {
    // Enlaces de eventos del DOM estándar
    ...
    // 1. Abrir explorador de archivos al hacer clic sin efecto bucle
    dropZone.addEventListener('click', () => {
        filePicker.click();
    });

    // 2. Capturar selección manual desde el explorador de archivos
    ...
    // 3. Prevenir comportamiento por defecto del navegador en toda la ventana
    window.addEventListener("dragover", (e) => e.preventDefault(), false);
    window.addEventListener("drop", (e) => e.preventDefault(), false);

    // 4. Gestionar arrastre sobre la zona interactiva (Animación visual)
...
/**
 * Procesa localmente el archivo arrastrado o seleccionado extrayendo el texto plano
 * @param {File} file 
 */
function processFile(file) {
    ...
    statusDiv.innerHTML = `<span class="text-primary fw-semibold"><i class="bi bi-file-earmark-arrow-up"></i> Leyendo archivo: ${file.name}...</span>`;

    if (nameLower.endsWith('.txt')) {
        const reader = new FileReader();
        reader.onload = function(e) {
            document.getElementById('rawText').value = e.target.result;
            statusDiv.innerHTML = `<span class="text-success fw-semibold"><i class="bi bi-check-circle-fill"></i> Texto de archivo TXT cargado con éxito.</span>`;
        };
            ...
            if (window.mammoth) {
                window.mammoth.extractRawText({ arrayBuffer: arrayBuffer })
                    .then(function(result) {
                        ...
                        statusDiv.innerHTML = `<span class="text-success fw-semibold"><i class="bi bi-check-circle-fill"></i> Texto de archivo DOCX extraído con éxito.</span>`;
                    })
                    .catch(function(err) {
                        statusDiv.innerHTML = `<span class="text-danger"><i class="bi bi-exclamation-triangle-fill"></i> Error al procesar DOCX: ${err.message}</span>`;
                    });
            } else {
                statusDiv.innerHTML = `<span class="text-warning"><i class="bi bi-exclamation-triangle-fill"></i> Motor Mammoth.js no cargado.</span>`;
            }
        };
        ...
        reader.onload = async function(e) {
            ...
            if (window['pdfjs-dist/build/pdf']) {
                try {
                    ...
                    statusDiv.innerHTML = `<span class="text-success fw-semibold"><i class="bi bi-check-circle-fill"></i> Texto de archivo PDF extraído con éxito.</span>`;
                } catch (err) {
                    statusDiv.innerHTML = `<span class="text-danger"><i class="bi bi-exclamation-triangle-fill"></i> Error al mapear PDF: ${err.message}</span>`;
                }
            } else {
                statusDiv.innerHTML = `<span class="text-warning"><i class="bi bi-exclamation-triangle-fill"></i> Motor PDF.js no inicializado.</span>`;
            }
        };
        reader.readAsArrayBuffer(file);
    } 
    else {
        statusDiv.innerHTML = `<span class="text-danger"><i class="bi bi-exclamation-triangle-fill"></i> Formato de archivo no soportado.</span>`;
    }
}
...
async function handleFormSubmitAI(e) {
    ...
    if (!apiKey) {
        alert("Se requiere una API Key de Gemini válida.");
        return;
    }
    if (!text) {
        alert("Incorpore texto en la caja de ingesta para continuar.");
        return;
    }

    localStorage.setItem('presulab_api_key', apiKey);
    const statusDiv = document.getElementById('aiStatus');
    statusDiv.innerHTML = `<span class="text-primary fw-semibold"><i class="bi bi-arrow-repeat d-inline-block animate-spin"></i> Gemini procesando el pedido...</span>`;
    ...
    systemPrompt += `   - Si las dimensiones vienen implícitas en centímetros (como 82.5, 70, 90, 120), conviértelas SIEMPRE a milímetros (mm) multiplicando por 10.\n`;
    ...
    systemPrompt += `3. Aísla de forma quirúrgica la descripción del material limpio ("description"). Elimina las cantidades y las medidas del texto final.\n`;
    systemPrompt += `4. Redacta notas técnicas concisas en el campo "observations" (Ej: Si incluye la palabra 'Carglas' indica "Especifica fabricante 'Carglas'"; si incluye un '+' añade "Vidrio laminado en una de sus caras").\n\n`;
    systemPrompt += `Devuelve ÚNICAMENTE un array JSON válido con la estructura abajo indicada. No incluyas bloques de código markdown tipo \`\`\`json.\n\n`;
    ...
    try {
        ...
        if (data.candidates && data.candidates[0].content.parts[0].text) {
            ...
            updateUserInterface();
            statusDiv.innerHTML = `<span class="text-success fw-bold"><i class="bi bi-check-circle-fill"></i> Datos distribuidos en los 4 módulos con éxito.</span>`;
        } else {
            statusDiv.innerHTML = `<span class="text-danger"><i class="bi bi-exclamation-triangle-fill"></i> El modelo rechazó la respuesta.</span>`;
        }
    } catch (error) {
        statusDiv.innerHTML = `<span class="text-danger"><i class="bi bi-exclamation-triangle-fill"></i> Error en el pipeline: ${error.message}</span>`;
        console.error(error);
    }
}
...
async function callGeminiSandbox() {
    ...
    if (!apiKey || !query) {
        alert("Requiere indicar la clave de acceso e introducir una instrucción.");
        return;
    }

    const outputDiv = document.getElementById('aiOutput');
    outputDiv.classList.remove('d-none');
    outputDiv.innerText = "Consultando al asesor técnico...";
    ...
    try {
        ...
        const data = await response.json();
        outputDiv.innerText = data.candidates[0].content.parts[0].text || "Sin respuesta del modelo.";
    } catch (e) {
        outputDiv.innerText = `Error de red: ${e.message}`;
    }
}

function copySqlToClipboard() {
    const code = document.getElementById('sqlOutputBlock').innerText;
    navigator.clipboard.writeText(code).then(() => alert("Código Transact-SQL copiado."));
}
...
function clearApiKey() {
    ...
    alert('Persistencia local eliminada.');
}
